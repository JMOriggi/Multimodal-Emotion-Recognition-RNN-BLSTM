...
from keras.layers import Flatten, Input, Dense, Masking, Dropout, LSTM, Bidirectional, Activation, Concatenate, Lambda
...
# --------------------------------------------------------------------------- #
#Main roots
mainRoot = os.path.normpath(r'C:\DATA\STUDIES\POLIMI\----TESI-----\Corpus\Corpus_All')
dirRes = os.path.normpath(r'C:\DATA\STUDIES\POLIMI\----TESI-----\Z_Results\KFOLD')
...
# DEFINE PARAMETERS
# --------------------------------------------------------------------------- #
flagModel = "Full" #Full #Solo
nb_lstm_cells = 128 #64 #128
nb_classes = 4
nb_hidden_units = 256 #256 #512
nb_hidden_units_2 = 512 #256 #512
nb_hidden_units_concat_layer = 256
n_epoch = 35 #35 #60 #number of epoch 
batchSize= 20
LRate = 0.001 #0,0001
#BALANCE LABELS AND WEIGHTS: coef1xNumb1=coef2xNumb2
labelLimit = 1080 #1080
...
def saveCsv(currentFileFeatures, name):
    csvOutputFilePath = os.path.join(dirRes, name + '.csv')
    try:
...
def buildBLTSM(numFeaturesAudio, numFeaturesText):
    ...
    x2 = Dense(nb_hidden_units_2, activation='relu')(x2)
    x2 = Dropout(0.5)(x2)
    x2 = Dense(nb_hidden_units_2, activation='relu')(x2)
    x2 = Dropout(0.5)(x2)
    y2 = Bidirectional(LSTM(nb_lstm_cells, return_sequences=True, dropout=0.5))(x2)
    #Attention step parallel for both model
    a1 = dot([u, y1], axes=-1)  # inner prod.
    a1 = Activation('softmax')(a1)
    a2 = dot([u, y2], axes=-1)  # inner prod.
    a2 = Activation('softmax')(a2)
    z1 = dot([a1, y1], axes=1)
    z2 = dot([a2, y2], axes=1)
    #Merge step
    mrg = Concatenate()([z1,z2]) #mrg = Concatenate([z1,z2])
    #Dense layer and final output
    refOut = Dense(nb_hidden_units_concat_layer, activation='relu')(mrg)
    ...
    model.compile(loss='categorical_crossentropy', optimizer=RMSprop(lr=LRate, rho=0.9, epsilon=None, decay=0.0), metrics=['categorical_accuracy']) #categorical_accuracy #mean_squared_error #categorical_crossentropy
...
def buildBLTSMSolo(numFeatures):
    input_attention = Input(shape=(nb_lstm_cells * 2,))
    u = Dense(nb_lstm_cells * 2, activation='softmax')(input_attention)
    input_feature = Input(shape=(None,numFeatures))#None
...
if __name__ == '__main__':
    ...
    for train_index, test_index in kfold.split(np.zeros(train_Audio.shape[0]),np.zeros(train_Audio.shape[0])):
        ...
        #print("Train set: " + str(train_index))
        #print("Test set: " + str(test_index))
        print("Length Train set: " + str(len(train_index)))
        print("Length Test set: " + str(len(test_index)))
        #Build Model
        if flagModel == "Solo":
            model = buildBLTSMSolo(allAudioFeature[0].shape[1])
            print("OK Solo")
        ...
        #Save training
        print("SAVE training: loss and accurancy")
        accArr = np.array(history.history['categorical_accuracy'])
        accArr = accArr.reshape(len(accArr), 1)
        lossArr = np.array(history.history['loss'])
        lossArr = lossArr.reshape(len(lossArr), 1)
        saveCsv(accArr, "trainingAcc"+str(foldIndex))
        saveCsv(lossArr, "trainingLoss"+str(foldIndex))
        #Save prediction
        print("SAVE PREDICTION & Expected")
        saveCsv(yhat, "prediction"+str(foldIndex))
        saveCsv(expected, "expected"+str(foldIndex))
        #PLOT & SAVE
