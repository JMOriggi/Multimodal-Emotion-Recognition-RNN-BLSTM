  ...
  npartial=7):
    ...
    for i in range(num_frames):
        ...
        while num_found < num_pitches and maybe <= ml.shape[0]:
            ...
            bounce1 = any([any([abs(ratio_to_cents(this_one/(other_one*harm))) < bounce_width for harm in bounce_ratios]) for other_one in bounce_list])
            bounce2 = any([any([abs(ratio_to_cents (other_one/(this_one*harm))) < bounce_width for harm in bounce_ratios]) for other_one in bounce_list])
            # if any bounces
        ...
        for bin_frq in ml_peaks:
            ...
            mask = np.abs(ratio_to_cents_protected((peaks_frqs/bin_frq), (peaks_frqs/bin_frq).round())) <= width
